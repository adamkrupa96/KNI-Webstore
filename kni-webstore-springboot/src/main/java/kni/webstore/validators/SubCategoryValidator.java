...
public class SubCategoryValidator implements Validator {
	...
	public void validate(Object objSubCategory, Errors errors) {
		...
		else subCategoryToValidate = (SubCategory) objSubCategory;
		...
		if (hasOnlyNumbers(subCategoryName)) errors.rejectValue("name", "only_digits_error", ONLY_DIGITS_ERROR_MSG );
		
		if ( !hasGoodLength(subCategoryName) ) 
			errors.rejectValue("name", "size_error", SIZE_ERROR_MSG);
		
		if (alreadyExists(subCategoryName)) errors.rejectValue("name", "exist_error", EXIST_ERROR_MSG );
	...
	private boolean alreadyExists(String subCategoryName) {
		...
		return allSubCategories.stream()
				.anyMatch(eachSubCategory -> eachSubCategory.getName().equals(subCategoryName));
	}
